            ...
            <ImageBackground
             ...
             source={require('../assets/pawprints.jpg')}>
                <View>
                    ...
                    <TextInput style={styles.questions} placeholder='Email' onSubmitEditing={Keyboard.dismiss} onChangeText={(username) => setUsername(username)} />
                    <TextInput style={styles.questions} placeholder='Password' onSubmitEditing={Keyboard.dismiss}  onChangeText = {(password) => setPassword(password)} />
                    ...
                    <Button title='Auth' onPress = { () => auth.createUserWithEmailAndPassword(username,password).then(creden =>{console.log(creden)}).then(navigation.navigate('MenuScreen'))}/>
